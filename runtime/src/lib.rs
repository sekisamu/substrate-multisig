...
use sp_runtime::traits::{
	BlakeTwo256, Block as BlockT, StaticLookup, IdentityLookup, Verify, ConvertInto, IdentifyAccount
};
...
impl multisig::Trait for Runtime {
	type Event = Event;
	type Proposal = Call;
	type AddrGenerator = multisig::MultiSigAddressGenerator<Self>;
}
	...
	impl frame_benchmarking::Benchmark<Block> for Runtime {
		fn dispatch_benchmark(
			...
			match module.as_slice() {
				...
				b"pallet-multisig" | b"multisig" => Multisig::run_benchmark(extrinsic, steps, repeat).ok(),
				_ => None,
			}
		}
	}
